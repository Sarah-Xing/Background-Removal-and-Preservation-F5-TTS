...
def add_noise(clean, noise, snr_db, eps=1e-10):
# clean, noise: (T,) tensors, same sample rate; pad/crop noise to len(clean)
    if noise.numel() < clean.numel():
        reps = (clean.numel() + noise.numel() - 1) // noise.numel()
        noise = noise.repeat(1, reps)[:,:clean.numel()]
    else:
        start = torch.randint(0, noise.numel() - clean.numel() + 1, (1,)).item()
        noise = noise[:,start:start+clean.numel()]
        print(f"Noise segment: {start} to {start+clean.numel()} of {noise.numel()}")

    # scale noise to achieve desired SNR
    clean_pow = clean.pow(2).mean().clamp_min(1e-12)
    noise_pow = noise.pow(2).mean().clamp_min(1e-12)
    target_noise_pow = clean_pow / (10**(snr_db/10))
    noise = noise * torch.sqrt(target_noise_pow / noise_pow)

    mixed = clean + noise
    # prevent clipping: peak norm or RMS norm
    peak = mixed.abs().max().clamp_min(1e-6)
    if peak > 0.99: mixed = mixed / peak * 0.99
    return mixed
